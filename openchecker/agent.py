...
def _execute_commands(command_list: list, project_url: str, res_payload: dict, commit_hash: str) -> None:
    ...
    for command in command_list:
        try:
            ...
            elif command == 'bestpractices-checker':
                _bestpractices_checker(project_url, res_payload)
            elif command == 'dangerous-workflow-checker':
                _dangerous_workflow_checker(project_url, res_payload)
...
def _dangerous_workflow_checker(project_url: str, res_payload: dict) -> None:
    """
    检查仓库中的危险工作流
    """
    ...
    def check_workflow_file(workflow_file: Path, repo_path: str) -> List[Dict[str, Any]]:
        """检查单个工作流文件"""
        ...
        def is_untrusted_ref(ref: str) -> bool:
            """检查引用是否不信任"""
            untrusted_patterns = [
        ...
        def find_dangerous_variables(script: str) -> List[str]:
            """查找脚本中的危险变量"""
            dangerous_vars = []
        ...
        def check_untrusted_checkout(workflow: Dict[str, Any], file_path: str) -> List[Dict[str, Any]]:
            """检查不信任代码检出"""
            dangerous_patterns = []
            # 检查是否使用危险触发器
        ...
        def check_script_injection(workflow: Dict[str, Any], file_path: str) -> List[Dict[str, Any]]:
            """检查脚本注入"""
            dangerous_patterns = []
            ...
            for job_id, job in jobs.items():
                ...
                for i, step in enumerate(steps):
                    ...
                    # 检查脚本中的危险变量
                    dangerous_vars = find_dangerous_variables(run_script)
        ...
        try:
            ...
            # 1. 检查不信任代码检出
            dangerous_patterns.extend(check_untrusted_checkout(workflow, relative_path))
            # 2. 检查脚本注入
            dangerous_patterns.extend(check_script_injection(workflow, relative_path))
        except Exception:
            # 忽略解析错误的文件
            pass
        return workflow_file_item
    ...
    logging.info(f"dangerous-workflow-checker processing projec: {project_url}")
    ...
    if not workflows_dir.exists():
        logging.info(f"dangerous-workflow-checker processing completed projec: {project_url}")
        res_payload["scan_results"]["dangerous-workflow-checker"] = workflows_file_detail
        return
    # 检查所有工作流文件
    for workflow_file in workflows_dir.glob("*.yml"):
        workflows_file_detail.append(check_workflow_file(workflow_file, repo_path))
